...
const connect = async (m, { conn, store }) => {  
   ...
   const isOwner = m.isLid ? (setting.ownerNumber.includes(mebot) || [...setting.ownerNumber, isMe].map(num => `${num}@lid`).includes(m.sender)) : [...setting.ownerNumber, isMe].map(num => `${num}@s.whatsapp.net`).includes(m.sender)  
   ...
   if (!db.users[m.sender]) db.users[m.sender] = { ...defaultUser };  
   if (!db.users[num]) db.users[num] = { ...defaultUser };  
   const users = db.users[m.sender];  
   const targetUser = db.users[num];  
   if (Object.keys(users).length < 1) db.users[m.sender] = { ...defaultUser };  
   ...
   if (targetUser.banned && !m.isBaileys && !m.fromMe) {  
      let reason = targetUser.bannedReason || 'Tanpa alasan';  
      if (cmd_plugins.includes(command)) {  
         return m.reply(mess.banned + reason);  
      } else {  
         return console.log(mess.banned + reason);  
      }  
   }  
